...
const CryptoActions = ({ crypto, onBuy, onSell }) => {
    ...
    const fetchUsdBalance = async () => {
        try {
            const response = await fetch('http://localhost:8081/api/v1/balance/current/usd', {
                method: 'GET',
                headers: {
    ...
    const fetchCryptoBalance = async () => {
        try {
            const response = await fetch(`http://localhost:8081/api/v1/balance/current/${crypto?.id}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${keycloak.token}`,
    ...
    const handleCredit = async () => {
        try {
            ...
            const response = await fetch('http://localhost:8081/api/v1/balance/credit', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${keycloak.token}`,
