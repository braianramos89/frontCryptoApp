...
const Home = () => {
        ...
        if (keycloak && keycloak.authenticated) {
            const fetchCryptos = async () => {
                try {
                    ...
                    const response = await fetch('http://cryptoapp:8081/api/v1/cryptos', {
                        method: 'GET',
                        headers: {
    ...
    const handleBuy = async (crypto, amount) => {
        try {
            const response = await fetch('http://cryptoapp:8081/api/v1/cryptos/buy', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${keycloak.token}`,
    ...
    const handleSell = async (crypto, amount) => {
        try {
            const response = await fetch('http://cryptoapp:8081/api/v1/cryptos/sell', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${keycloak.token}`,
